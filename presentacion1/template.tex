...
\begin{frame}
  ...
  \justifying
  \begin{itemize}
    \item \scriptsize Dada un área de interés desconocida en un espacio cerrado que se desea explorar denotada como $\mathcal{A}$, tal que $\mathcal{A} \subseteq \mathbb{R}^{3}$.\\
    \item \scriptsize Un voxel \textit{v} que representa el espacio contenido en $\mathcal{A}$ que es obtenido dividiendo recursivamente el área de interés $\mathcal{A}$ en ocho partes de igual tamaño, el voxel puede tomar los valores de libre o ocupado denotados como $v_{libre}$, $v_{occ}$.\\
    \item \scriptsize Un conjunto de VANTS autónomos denotado como $\mathcal{V}=\{V_1,V_2,V_3,...,V_n\}$, siendo $n$ el número total de VANTS disponibles, así como una configuración inicial q cuya cardinalidad es el número de VANTS disponibles denotado como $q=\{q_{1},q_{2},q_{3},...,q_{n}\}$.\\
  \end{itemize}
  
  \scriptsize La región ocupada son obtenidas mediante un sensor basado en un modelo de ocupación probabilístico.\\
  \bigskip % Vertical whitespace
  \scriptsize Determinar el conjunto de tareas para cada VANT, así como el conjunto de rutas que maximize el área explorada minimizando el tiempo y la energía consumida.\\
  \bigskip % Vertical whitespace
\end{frame}

\begin{frame}{Preguntas de investigación}
  Con base en lo anterior, surgen las siguiente preguntas de investigación:
  \begin{itemize}
  \item ¿Qué estrategias pueden emplearse para optimizar la asignación de tareas y coordinación entre los múltiples VANTS para el problema de exploración de un ambiente desconocido en interiores sin señal GPS?
  \item ¿Cómo se pueden diseñar algoritmos rápidos para la planificación de rutas que guie de forma efectiva los múltiples VANTS, considerando la evasión de obstáculos, optimizando la cobertura del área a explorar?
  \item ¿Qué mecanismos de coordinación y comunicación pueden facilitar la colaboracion y el intercambio rápido de información entre múltiples VANTS durante las misiones de exploración?
  \item ¿Cómo asegurar que la información que aporte cada uno de los VANTS se integre correctamente a la información ya conocida para generar mapas precisos y completos del entorno explorado?
  \end{itemize}
