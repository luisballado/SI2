...
\begin{frame}
  ...
  %  \column{0.5\textwidth}
  \justifying
  \small Dado un volumen de interés desconocida en un espacio cerrado que se desea explorar denotada como $\mathcal{E}$, tal que $\mathcal{E} \subset \mathbb{R}^{3}$.\\

  \small El espacio $\mathcal{E}$ puede ser representado usando una estructura de datos tridimencional de tipo árbol. A partir de los valores de una cámara RGB-D basada en un modelo de ocupación probabilístico que puede tomar los valores de libre, ocupado y desconocido o no explorado de notados como $v_{libre}$, $v_{ocup}$, $v_{desc}$.
  
  \begin{itemize}
  \item Un conjunto de VANTS denotado como $\mathcal{V} = \{\mathcal{V}_{1},\mathcal{V}_{2},\mathcal{V}_{3},...,\mathcal{V}_{n}\}$ siendo $n$ el número total de VANTS disponibles.
  \item Una configuración inicial $q$ cuya cardinalidad es el número de VANTS disponibles denotado como $q = \{q_{1},q_{2},q_{3},...,q_{n}\}$.\\
  \end{itemize}
  ...
  \bigskip % Vertical whitespace
  El problema se basa en la propuesta de una estrategia que incluya las habilidades de autonomía, para tareas de exploración de forma coordinada dividiendo la carga de trabajo de exploración entre el grupo.\\
...
\begin{frame}{Preguntas de investigación}
  ...
  \item ¿Es posible que el uso de una estructura de datos como octree para representar la ocupación de un volumen, sea más eficiente que una representación en una matriz cúbica?
    %\pause
...
\begin{frame}{Hipótesis}
  ...
  Una estrategia de coordinación y asignación de tareas en combinación con una arquitectura de software (que resuelve los problemas de planificación de rutas, evasión de obstáculos y localización) puede mejorar la exploración de múltiples VANTS en entornos desconocidos tomado un enfoque descentralizado.
...
\begin{frame}{Objetivos generales y específicos del proyecto}
  ...
  \begin{enumerate}
    ...
    \item General \\
    \bigskip
    Desarrollar una arquitectura de software que resuelva los problemas de localización, mapeo, navegación y coordinación multi-VANT en ambientes desconocidos para tareas de exploración en interiores.
    \pause
    % \item<2-> Particulares\\
    \item Particulares\\
    \bigskip
...
\begin{frame}
  ...
  \item<1-> Documentación y códigos liberados
    \begin{itemize}
    \item Algoritmo para la asignación de tareas de exploración multi-VANT.
    \item Arquitectura de software que resuelva los problemas de autonomía para un vehículo aéreo no tripulado.
    \item Protocolo de comunicación y coordinación multi-VANT que formaran parte de la arquitectura de software
    \end{itemize}
